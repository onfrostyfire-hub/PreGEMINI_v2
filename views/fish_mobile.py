...
def generate_mobile_theme(bg_rad1, bg_rad2, shadow1, shadow2, shadow3, seat_rad, seat_border, seat_act_border, seat_act_shadow, anim_name, pulse_shadow1, pulse_shadow2, text_color, badge_bg, bar_fill, card_bg, card_border, rng_bg):
    ...
    /* ИКОНКА ONENOTE */
    .info-hook {{ position: absolute; top: auto; bottom: -30px; left: calc(50% + 70px); width: 22px; height: 22px; background: rgba(0,0,0,0.62); border: 1.5px solid rgba(255,255,255,0.36); border-radius: 50%; color: #fff; display: flex; justify-content: center; align-items: center; font-weight: bold; font-family: serif; text-decoration: none; transition: 0.2s; z-index: 100; font-size: 12px; text-shadow: none; box-shadow: 0 3px 8px rgba(0,0,0,0.55); }}
...
def show():
    st.markdown("""
        ...
        .pf-pot-badge { position: absolute; top: 25.5%; left: 50%; transform: translateX(-50%); z-index: 15; }
        .pf-street-pot { position: absolute; top: 56.5%; left: 50%; transform: translateX(-50%); z-index: 18; display: flex; flex-direction: row; align-items: center; justify-content: center; gap: 6px; pointer-events: none; }
        .pf-board { position: absolute; top: 43.5%; left: 50%; transform: translate(-50%, -50%); z-index: 15; }
        
        .mobile-game-area { margin: 28px auto 36px auto !important; }
        ...
        div.element-container:has(#fish-runouts-all-m-marker) + div.element-container div[data-testid="stButton"] { width: 48px !important; margin-left: auto !important; }
        div.element-container:has(#fish-runouts-all-m-marker) + div.element-container div[data-testid="stButton"] button { width: 48px !important; height: 24px !important; min-height: 24px !important; padding: 0 !important; border-radius: 8px !important; background: #242932 !important; border: 1px solid rgba(255,255,255,0.18) !important; box-shadow: none !important; }
        ...
        .street-chip-pile { position: relative; width: 27px; height: 24px; filter: drop-shadow(0 3px 5px rgba(0,0,0,0.72)); }
        .street-chip-pot { position: absolute; width: 16px; height: 16px; left: 5px; border-radius: 50%; border: 1.5px solid rgba(255,255,255,0.24); background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #5ee08a, #145c32 72%); box-shadow: 0 0 0 1.2px rgba(0,0,0,0.72), 0 2px 4px rgba(0,0,0,0.7), inset 0 1px 2px rgba(255,255,255,0.24); }
        .street-chip-pot::before { content: ''; position: absolute; inset: 3.5px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.16); background: rgba(0,0,0,0.06); }
        .street-chip-pot::after { content: ''; position: absolute; top: 2px; left: 3px; width: 6px; height: 4px; border-radius: 50%; background: rgba(255,255,255,0.24); filter: blur(0.6px); }
        .street-chip-pot.c1 { top: 8px; left: 3px; background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #5ee08a, #145c32 72%); }
        .street-chip-pot.c2 { top: 6px; left: 7px; background: repeating-conic-gradient(rgba(255,255,255,0.2) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #ef4444, #7f1220 72%); }
        .street-chip-pot.c3 { top: 4px; left: 3px; background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #60a5fa, #163f86 72%); }
        ...
        .spot-mastery-felt { position: absolute; top: 63.5%; left: 50%; transform: translateX(-50%); z-index: 16; min-width: 142px; display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 0; pointer-events: none; background: transparent; border: 0; box-shadow: none; }
        ...
        .spot-mastery-meter { display: block; width: 104px; height: 3px; border-radius: 999px; overflow: hidden; background: rgba(255,255,255,0.09); box-shadow: inset 0 1px 2px rgba(0,0,0,0.78), 0 0 8px rgba(56,220,160,0.1); }
        .spot-mastery-fill { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, rgba(54,208,130,0.88), rgba(244,211,132,0.94)); box-shadow: 0 0 9px rgba(72,220,150,0.3); }
        .spot-mastery-remaining { font-size: 9px; font-weight: 800; letter-spacing: 0.03em; line-height: 1; color: rgba(220,232,224,0.58); text-shadow: 0 1px 5px rgba(0,0,0,0.82); white-space: nowrap; }
        .spot-line-felt { position: absolute; top: 18.5%; left: 50%; transform: translateX(-50%); z-index: 14; width: 86%; text-align: center; pointer-events: none; color: rgba(206,224,214,0.27); font-family: 'Roboto', sans-serif; font-size: 13.5px; font-weight: 900; letter-spacing: 0.04em; line-height: 1; text-shadow: 0 1px 3px rgba(0,0,0,0.46); white-space: nowrap; }
        ...
        .hero-mob { position: absolute !important; bottom: -64px !important; left: 50% !important; transform: translateX(-50%) !important; z-index: 30 !important; display: flex !important; flex-direction: column !important; align-items: center !important; gap: 4px !important; width: 120px !important; }
        ...
        .rng-badge { display: none !important; }
    ...
    if not fish_db: 
        st.error("Database is empty. Check JSON files in fish_data.")
        return
    ...
    with st.expander("⚙️ Fish Filters", expanded=False):
        ...
        if lines_for_filters:
            ...
            with all_col:
                st.markdown('<span id="fish-runouts-all-m-marker"></span>', unsafe_allow_html=True)
                if matching_runouts and st.button("ALL", key="fish_runouts_all_m", use_container_width=False):
                    target_state = not all_runouts_selected
                    for _, full_key in matching_runouts:
                        st.session_state[f"fish_chk_m_{full_key}"] = target_state
    ...
    is_flashing = "rage-flash" if st.session_state.pop("fish_just_leveled_up", False) else ""
...
"""
    ...
    if is_hu:
        ...
        if is_bet:
            ...
            bet_txt = f'<div class="bet-txt">{bet_amount_str} bb</div>'
    ...
    if hero_act:
        ...
        if is_hero_bet:
            ...
            hero_bet_txt = f'<div class="bet-txt">{hero_bet_amount_str} bb</div>'
    ...
    if is_flashing_correct:
        ...
        st.markdown(f'<div style="background:#dc3545; color:white; padding:12px; border-radius:12px; text-align:center; font-weight:bold; margin-bottom:15px; font-size:16px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">{st.session_state.fish_msg}</div>', unsafe_allow_html=True)
        st.markdown("""<style>
        ...
        st.markdown(f'<div class="rng-hint-wrap">{spot_line_name}</div>', unsafe_allow_html=True)
