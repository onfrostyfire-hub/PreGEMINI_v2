...
def apply_preflop_filters(df_hist):
    ...
    render_chip_group(
        ...
        state_key="stats_filters_desk_pre_scenario",
        key_prefix="desk_pre_scenario",
        columns_per_row=4,
        sorter=scenario_sort_key,
    )

    active_categories = set(st.session_state.get("stats_filters_desk_pre_scenario", []))
...
def apply_postflop_filters(df_hist):
    ...
    render_chip_group(
        ...
        state_key="stats_filters_desk_pf_spot",
        key_prefix="desk_pf_spot",
        columns_per_row=4,
    )
    render_chip_group(
        title="Hero",
        items=sorted(df_hist["PF_Hero"].dropna().unique(), key=lambda item: item.lower()),
        state_key="stats_filters_desk_pf_hero",
        key_prefix="desk_pf_hero",
        columns_per_row=4,
    )
    render_chip_group(
        title="Street",
        items=sorted(df_hist["PF_Street"].dropna().unique(), key=lambda item: item.lower()),
        state_key="stats_filters_desk_pf_street",
        key_prefix="desk_pf_street",
        columns_per_row=4,
    )
    render_chip_group(
        title="Branch",
        items=sorted(df_hist["PF_Branch"].dropna().unique(), key=lambda item: item.lower()),
        state_key="stats_filters_desk_pf_branch",
        key_prefix="desk_pf_branch",
        columns_per_row=4,
    )

    active_spots = set(st.session_state.get("stats_filters_desk_pf_spot", []))
    active_heroes = set(st.session_state.get("stats_filters_desk_pf_hero", []))
    active_streets = set(st.session_state.get("stats_filters_desk_pf_street", []))
    active_branches = set(st.session_state.get("stats_filters_desk_pf_branch", []))
...
def clear_filter_state(is_postflop):
    if is_postflop:
        for key in [
            "stats_filters_desk_pf_spot",
            "stats_filters_desk_pf_hero",
            "stats_filters_desk_pf_street",
            "stats_filters_desk_pf_branch",
        ]:
            st.session_state[key] = []
    else:
        st.session_state["stats_filters_desk_pre_scenario"] = []
...
def show():
    st.markdown(
        ...
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(2) { flex: 0 0 54px !important; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(3) { flex: 1 1 35% !important; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(4) { flex: 0 0 42px !important; text-align: right; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(5) { flex: 1 1 35% !important; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(6) { flex: 0 0 36px !important; text-align: right; }
    ...
    col_1.metric("Total Hands", total_hands)
    ...
    reset_col_1, reset_col_2 = st.columns([6, 1.4])
    with reset_col_2:
        if st.button("RESET FILTERS", key=f"desk_reset_{mode.lower()}", use_container_width=True):
    ...
    with train_col:
        ...
        if st.button("TRAIN SELECTED", key=f"desk_train_selected_{mode.lower()}", use_container_width=True):
    ...
    for spot_name, count in sorted_spots:
        ...
        row_col_1, row_col_2, row_col_3, row_col_4, row_col_5, row_col_6 = st.columns(6)
        ...
        with row_col_2:
            ...
            st.markdown("</div>", unsafe_allow_html=True)
        with row_col_3:
            st.markdown(f"<div class='mastery-name' title='{spot_name}'>{spot_name}</div>", unsafe_allow_html=True)
        with row_col_4:
        ...
        with row_col_5:
            st.markdown(
                (
                    ...
                    f"<div class='mastery-bar-fill' style='width:{pct}%; background:{gradient}; box-shadow:0 0 10px {glow};'></div>"
                    "</div>"
                ),
                unsafe_allow_html=True,
            )
    ...
    with st.expander("Recover Spot Mastery from History", expanded=False):
        ...
        if st.button("RECOVER SPOT MASTERY", key=f"desk_recover_{mode.lower()}", use_container_width=True):
