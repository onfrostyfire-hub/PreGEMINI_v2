...
def show():
    st.markdown("""
        ...
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) {
            ...
            align-items: center !important; background: rgba(28,28,30,0.6) !important; padding: 10px 14px !important;
            border-radius: 12px !important; border: 1px solid rgba(255,255,255,0.05) !important; margin-bottom: 8px !important;
            gap: 12px !important; width: 100% !important; box-shadow: 0 4px 10px rgba(0,0,0,0.15) !important;
        }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"] { margin: 0 !important; padding: 0 !important; min-width: 0 !important; width: auto !important; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(1) { flex: 0 0 24px !important; }
        div[data-testid="stHorizontalBlock"]:has(.spot-row-marker-desk) > div[data-testid="column"]:nth-child(2) { flex: 0 0 36px !important; }
        ...
        /* MASTERY BARS DESKTOP */
        .mastery-name { color: #f2f2f7; font-weight: 600; font-size: 13px; letter-spacing: -0.1px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin: 0; line-height: 1.2; }
        .mastery-count { color: #fff; font-weight: 800; font-size: 14px; font-variant-numeric: tabular-nums; margin: 0; line-height: 1.2; }
        .mastery-max { color: #6c757d; font-size: 11px; font-weight: 700; margin: 0; line-height: 1.2; }
        .mastery-bar-container { width: 100%; background: rgba(0,0,0,0.5); height: 8px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 1px 3px rgba(0,0,0,0.8); margin-top: 2px; }
        .mastery-bar-fill { height: 100%; border-radius: 4px; transition: width 0.5s cubic-bezier(0.25, 1, 0.5, 1); }
    ...
    st.markdown("### 🎯 Filters")
    filter_key = f"active_filters_v7_desk_{is_postflop}"
    ...
    if available_cats:
        ...
        for i, cat in enumerate(available_cats):
            with cols[i]:
                ...
                if st.button(cat, key=f"f_desk_{cat}", type="primary" if is_active else "secondary"):
