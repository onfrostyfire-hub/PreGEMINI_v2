...
def show():
    st.markdown("""
        ...
        .cn-desk-header { max-width: 850px; margin: -4px auto 4px auto; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif; -webkit-font-smoothing: antialiased; border-radius: 16px; overflow: hidden; position: relative; background: linear-gradient(165deg, rgba(18,22,28,0.92) 0%, rgba(8,10,14,0.96) 100%); border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 1px 0 rgba(255,255,255,0.06) inset, 0 8px 32px rgba(0,0,0,0.55), 0 0 0 1px rgba(0,0,0,0.4); backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); }
        ...
        .cn-desk-wr-track { height: 2px; width: 100%; background: rgba(0,0,0,0.45); position: relative; z-index: 1; }
        .cn-desk-wr-fill { height: 100%; transition: width 0.35s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 0 12px currentColor; }
        .cn-desk-inner { position: relative; z-index: 1; padding: 9px 18px 10px; }
        .cn-desk-row1 { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 6px; }
        ...
        .rage-labels-desk { position: absolute; left: 0; right: 0; top: 50%; transform: translateY(-50%); display: flex; justify-content: space-between; align-items: center; padding: 0 14px; pointer-events: none; z-index: 4; font-family: 'Inter', system-ui, sans-serif; font-weight: 800; font-size: 12px; font-variant-numeric: tabular-nums; letter-spacing: 0.02em; color: rgba(255,255,255,0.98); text-shadow: 0 1px 2px rgba(0,0,0,0.95), 0 0 12px rgba(0,0,0,0.8), 0 0 1px rgba(0,0,0,1); }
        ...
        .street-chip-pile-desk { position: relative; width: 32px; height: 28px; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.75)); }
        .street-chip-pot-desk { position: absolute; width: 20px; height: 20px; left: 6px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.24); background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #5ee08a, #145c32 72%); box-shadow: 0 0 0 1.5px rgba(0,0,0,0.72), 0 2px 5px rgba(0,0,0,0.72), inset 0 1px 2px rgba(255,255,255,0.24); }
        .street-chip-pot-desk::before { content: ''; position: absolute; inset: 4px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.16); background: rgba(0,0,0,0.06); }
        .street-chip-pot-desk::after { content: ''; position: absolute; top: 3px; left: 4px; width: 7px; height: 4px; border-radius: 50%; background: rgba(255,255,255,0.24); filter: blur(0.7px); }
        .street-chip-pot-desk.c1 { top: 8px; left: 4px; background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #5ee08a, #145c32 72%); }
        .street-chip-pot-desk.c2 { top: 6px; left: 8px; background: repeating-conic-gradient(rgba(255,255,255,0.2) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #ef4444, #7f1220 72%); }
        .street-chip-pot-desk.c3 { top: 4px; left: 4px; background: repeating-conic-gradient(rgba(255,255,255,0.22) 0deg 16deg, transparent 16deg 34deg), radial-gradient(circle at 36% 30%, #60a5fa, #163f86 72%); }
    ...
    with st.sidebar:
        ...
        with c_due:
            ...
            if st.button(due_label, key="fish_due_mode_d", use_container_width=True):
    ...
    if review_ctx:
        ...
        if review_pool:
            ...
            if st.sidebar.button("Exit Review", key="exit_review_fish_d", use_container_width=True):
    ...
    mastery_html = f"""<div class="spot-mastery-felt-desk">
        <div class="spot-mastery-pill-desk">{m_icon}<span>{m_name}</span></div>
        <div class="spot-mastery-meter-desk"><div class="spot-mastery-fill-desk" style="width: {m_pct}%;"></div></div>
