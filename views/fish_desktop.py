...
def get_card_html(card_str):
    ...
    <div style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); 
                border-radius: 8px; width: 45px; height: 65px; display: flex; flex-direction: column; 
                justify-content: center; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,0.3); margin: 0 2px;">
        <div style="color: {color}; font-size: 20px; font-weight: 900; line-height: 1;">{rank}</div>
        <div style="color: {color}; font-size: 22px; line-height: 1;">{sym}</div>
    </div>
    '''
...
def show():
    ...
    st.markdown("""
    <style>
        .fish-table { background: radial-gradient(circle at center, #1a2a3a 0%, #0d151c 100%); border-radius: 15px; padding: 20px; box-shadow: inset 0 0 50px rgba(0,0,0,0.8), 0 10px 20px rgba(0,0,0,0.5); border: 2px solid #2c3e50; position: relative; margin-top: 10px; }
        ...
        .fish-btn button { background: rgba(255,255,255,0.05) !important; border: 1px solid rgba(255,255,255,0.2) !important; color: #fff !important; padding: 15px !important; border-radius: 10px !important; font-weight: bold !important; transition: 0.2s !important; height: 60px !important; }
        .fish-btn button:hover { background: rgba(255,255,255,0.1) !important; border-color: #ffc107 !important; color: #ffc107 !important; }
    ...
    with col2:
        ...
        st.markdown(f"""
        <div style="display:flex; justify-content:space-between; background:#111; padding:10px 20px; border-radius:10px; border:1px solid #333; margin-bottom:10px;">
            <div><span style="color:#aaa; font-size:12px;">COMBO</span><br><span style="color:#ffc107; font-size:20px; font-weight:900;">x{c_rnk}</span></div>
            <div><span style="color:#aaa; font-size:12px;">SHIELDS</span><br><span style="color:#0dcaf0; font-size:20px; font-weight:900;">{c_sh}</span></div>
            <div style="text-align:right;"><span style="color:#aaa; font-size:12px;">XP</span><br><span style="color:#28a745; font-size:20px; font-weight:900;">{c_xp}</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        chip_html = f'<div class="chip">{v_size}bb</div>' if v_size else ""
        ...
        <div class="fish-table {fb_class}">
            {hook_html}
            {chip_html}
            <div style="text-align:center; color:#adb5bd; font-size:12px; font-weight:bold; letter-spacing:1px; margin-bottom:15px;">
                {spot['vpip']} | {spot['pos']} | {spot['line']}
            </div>
            
            <div style="display:flex; justify-content:center; align-items:center; min-height:80px; margin-bottom:20px;">
                {board_html}
            </div>
            
            <div style="text-align:center; margin-bottom:20px;">
                <div style="color:#6c757d; font-size:12px; font-weight:bold; margin-bottom:5px;">POT: {setup.get("pot_size", 0)} BB</div>
                {v_badge}
            </div>
            
            <div style="display:flex; justify-content:center; align-items:center; margin-top:20px;">
                <div style="margin-right:15px; color:#aaa; font-size:10px; font-weight:bold;">HERO<br>{setup.get("hero_pos", "UKN")}</div>
                {hero_html}
            </div>
        </div>
        {fb_text}
        ...
        if actions:
            ...
            for idx, act in enumerate(actions):
                with cols[idx]:
                    ...
                    if st.button(act, key=f"btn_{act}_{spot['runout']}_{st.session_state.fish_stats['total_hands']}", use_container_width=True):
                        handle_action(act)
                        generate_spot()
                        st.rerun()
                    st.markdown('</div>', unsafe_allow_html=True)
