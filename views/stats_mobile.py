...
def show():
    ...
    if not sorted_spots:
        ...
        if st.button("TRAIN SELECTED", key=f"train_selected_mobile_{mode.lower()}", use_container_width=True):
            selected_spots = [spot_name for spot_name, _ in sorted_spots if st.session_state.get(f"sel_mobile_{mode.lower()}_{spot_name}", False)]
        ...
        for spot_name, count in sorted_spots:
            ...
            with left_col:
                ...
                st.checkbox(" ", key=f"sel_mobile_{mode.lower()}_{spot_name}")
                st.markdown("</div>", unsafe_allow_html=True)

            with action_col:
                st.markdown('<div class="target-btn">', unsafe_allow_html=True)
                if st.button("GO", key=f"go_mobile_{mode.lower()}_{spot_name}", use_container_width=True):
            ...
            with card_col:
                ...
                html_output = (
                    ...
                    f"<div style='flex:0 0 auto; color:#6c757d; font-weight:700; font-size:10px;'>{road_label}</div>"
                    "</div>"
                )
